...
{        
     ...
     */
    public function getCheckList($delete=0, $columns,$rows,$link=null,$target=null,$editLink="", $class='items', $textalign= "left", $report=false, $id = "exportExcel")
    {
    	$tr = Application_Form_FrmLanguages::getCurrentlanguage();
    	/*
		  				...
		  				if($i==0) {
    						...
    						if($link!=null){
    							foreach($link as $column=>$url)
    								if($key==$column){
    									$img='';
    									$array=array('tag'=>'a','attribute'=>array('href'=>Application_Form_FrmMessage::redirectorview($url).'/id/'.$temp,'target'=>$target));
    									$read=$this->formSubElement($array,$img.$read);
    								}
    						}
    						$text='';
